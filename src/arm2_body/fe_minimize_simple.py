...
class FE_minimize:
    ...
    def iter(self):
        ...
        #mu_dot_vis = 0.0 * dF_dmu_vis    # mu_dot_vis is a 1x4 vector
...
#     diff1 = fe_min.a[0][0]*fe_min.dt
#     targ1 = q[0]+ diff1

#     diff2 = fe_min.a[0][1]*fe_min.dt
#     targ2 = q[1]+ diff2
#     # targ = q[i]+ diff
#     # d.qpos[i] = targ
#     # if abs(diff)>=diff_thres:
#     if targ1<larm_max[0] and targ1>larm_min[0]:
#         d.ctrl[0] = targ1
...
#     else:
#         targ1=np.minimum(larm_max[0],np.maximum(targ1, larm_min[0]))
#         if abs(targ1-q[0])>=diff_thres:
#             d.ctrl[0] = targ1
#             #d.qvel[1] = fe_min.a[0][0]
#         #print('Joints limit reached in: {}'.format(actors[0]))
    
#     if targ2<larm_max[1] and targ2>larm_min[1]:
#         d.ctrl[1] = targ2
#         #d.qvel[1] = fe_min.a[0][0]
#         #print(d.qpos[1])
#         #print("\n")
#         #print("Action has been carried out")
#     else:
#         targ2=np.minimum(larm_max[1],np.maximum(targ2, larm_min[1]))
#         if abs(targ2-q[1])>=diff_thres:
#             d.ctrl[1] = targ2
#             #d.qvel[1] = fe_min.a[0][0]
#         #print('Joints limit reached in: {}'.format(actors[0]))
# # #print('Last target {}'.format(self.last_target))
        ...
        <body name="foundation" pos="0 0 0.1">
            ...
            <body name="pole" pos="0 0 0">
                ...
                <body name="pole2" pos="0 1.2 0">
                    ...
                    <geom fromto="0 0 0 0 .8 0" name="cpole2" rgba="0.0 0.7 0.7 1" size="0.045 0.3" type="capsule"/>
                </body>
            </body>
        </body>
	</worldbody>
...
#   print((fe_min.mu).shape)
  ...
  while viewer.is_running() and time.time() - start < sim_len:
    ...
    if n == 0:
        start = time.time()
    # time constant check
    if np.abs(d.qpos[1]) >= 0.63 * np.abs(targ_q1[1]) and flg_timeconst1:
        time_const1 = time.time() - start
        flg_timeconst1 = 0
    if np.abs(d.qpos[1]) >= 0.8 * np.abs(targ_q1[1]) and flg_timeconst2:
        time_const2 = time.time() - start
        flg_timeconst2 = 0
    if np.abs(d.qpos[1]) >= 0.9 * np.abs(targ_q1[1]) and flg_timeconst3:
        time_const3 = time.time() - start
        flg_timeconst3 = 0
    if np.abs(d.qpos[1]) >= 0.95 * np.abs(targ_q1[1]) and flg_timeconst4:
        time_const4 = time.time() - start
        flg_timeconst4 = 0
    print(d.qpos[0])
    mujoco.mj_step(m, d)
    print(d.qpos[0])
...
for i in range(1, 3):
    ax = fig.add_subplot(1, 2, i)
    ax.set_title(name_list[i-1], fontsize=30)
    ax.plot(t, log_mu[i-1], label=lg_lbl_mu[i-1])
    ax.plot(t, log_q[i-1], color="black")
    ax.plot(t[::marker_interval], log_q[i-1][::marker_interval], linestyle='', marker='o',markersize=10, color="black")
    ax.plot(t, np.array(log_q[i-1])+5, linestyle='-', marker='o',markersize=10, color="black", label=lg_lbl_theta[i-1])
...
for i in range(1, 3):
    ...
    ax.set_title(name_list[i-1], fontsize="30")
    #ax.plot(t, log_mu[i-1], label=lg_lbl_mu[i-1])
    ax.plot(t, log_q[i-1], label=lg_lbl_theta[i-1], color="tab:orange", linewidth=3)
...
for i in range(1, 3):
    ...
    ma, std = moving_average_std(log_attr_err[i-1], 50)
    # 時系列グラフをプロット
    ax = fig.add_subplot(1, 2, i)
    ax.set_title(name_list[i-1])
    ax.plot(range(len(log_attr_err[i-1]) - 50 + 1), ma, label='Attr Error Average')
    ax.fill_between(range(len(log_attr_err[i-1]) - 50 + 1), np.array(ma) - np.array(std), np.array(ma) + np.array(std), color='b', alpha=0.2)
# グラフを表示
plt.legend(fontsize="15")
plt.show()
...
for i in range(1, 3):
    # 10データごとの移動平均と標準偏差を計算
    ma, std = moving_average_std(log_vis_err[i-1], 50)
    # 時系列グラフをプロット
    ax = fig.add_subplot(1, 2, i)
    ax.set_title(name_list[i-1])
    ax.plot(range(len(log_vis_err[i-1]) - 50 + 1), ma, label='Vis Error Average')
    ax.fill_between(range(len(log_vis_err[i-1]) - 50 + 1), np.array(ma) - np.array(std), np.array(ma) + np.array(std), color='b', alpha=0.2)
# グラフを表示
plt.legend(fontsize="15")
plt.show()
...
# save movie
media.write_video("movie.mp4", frames, fps=60)


# time_steps = np.linspace(0, time_len, len(log_q[0]))

# # アニメーションの作成
# fig, axes = plt.subplots(1, 3, figsize=(15, 5))

# # 関節角のグラフ用設定
# line1, = axes[0].plot([], [], lw=2, label='Joint 1')
# line2, = axes[0].plot([], [], lw=2, label='Joint 2')
# axes[0].set_xlim(0, time_len)
# axes[0].legend()

# # 動画フレーム用（初期フレームの表示）
# img_ax = axes[1].imshow(np.zeros((240, 320)), aspect='auto')  # 初期サイズは動画の解像度に依存
# axes[1].axis('off')  # 軸を非表示に
...
#     line1.set_data([], [])
#     line2.set_data([], [])
#     img_ax.set_array(np.zeros((240, 320)))  # 適切なサイズに初期化
#     return (line1, line2, img_ax)
...
#     # 動画フレームの更新
#     img_ax.set_array(frames[frame])  # フレームリストから画像を設定
    
#     return (line1, line2, img_ax)

# ani = FuncAnimation(fig, update, frames=len(time_steps), init_func=init, blit=True)
...
for i in range(10000):
    image_path = f"{image_folder}/{i+1}.png"  # 画像ファイルのパスを指定
...
# 100x100のMSE行列を初期化
mse_matrix = np.zeros((100, 100))


# 画像同士のMSEを計算して行列に格納
for i in range(100):
    for j in range(100):
        mse = ((image_list[100*i + j] - targ_img)**2).mean()
        mse_matrix[99-j, i] = mse
        print(100*i+j)
...
def plot_heatmap(ax, mse_matrix):
    ax.imshow(mse_matrix, cmap='RdBu')
    ax.set_xlabel("angle of joint1 [rad]")
    ax.set_ylabel("angle of joint2 [rad]")
...
plot_heatmap(ax, mse_matrix)  # ヒートマップをプロット
...
# 動画として保存 (ffmpegが必要)
ani.save("error_field.mp4", writer='ffmpeg', dpi=dpi)
plt.show()
...
# ヒートマップを作成
plt.imshow(mse_matrix, cmap='RdBu')
plt.colorbar()  # カラーバーを追加
# ヒートマップの勾配を計算
grad_y, grad_x = np.gradient(mse_matrix)

# ヒートマップ上に矢印を描画
# 'X' と 'Y' は矢印の位置、'U' と 'V' は矢印の方向と大きさ
Y, X = np.mgrid[0:100:5, 0:100:5]
plt.quiver(X, Y, -grad_x[::5, ::5], -grad_y[::5, ::5], color='white')
plt.xlabel(r"$\theta_1 [rad]")
plt.ylabel(r"$\theta_2 [rad]")
#plt.plot(path_x[-1], path_y[-1], marker='.', markersize=10, color="red")
plt.xlim([0,100])
#plt.ylim([0,100])
...
# plt.colorbar()  # カラーバーを追加
plt.savefig("error_field.png", dpi=300)
plt.show()



print("time constant1")
print(time_const1)
print("time constant2")
print(time_const2)
print("time constant3")
print(time_const3)
print("time constant4")
print(time_const4)