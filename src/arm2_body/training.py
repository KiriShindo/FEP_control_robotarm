...
train_image_paths = [os.path.join(train_image_folder, f'{filename}.png') for filename in range(1,10001)]
...
test_image_paths = [os.path.join(test_image_folder, f'{filename}.png') for filename in range(1,901)]
...
# モニタリング用
random_vectors = torch.tensor([-0.698, -0.698])
...
torch.save(params, "net_1000.prm", pickle_protocol=4)
...
np.savetxt("loss/train_loss_e1000.csv", train_loss)
np.savetxt("loss/test_loss_e1000.csv", test_loss)
